      ...
      <div className="w-full max-w-md relative z-10 space-y-6">
        ...
        <div className="flex flex-col items-center text-center space-y-3">
          ...
          <div className="space-y-1">
            <div className="inline-flex items-center gap-2 rounded-full border border-pink-200 bg-pink-50/80 px-3 py-1 text-[11px] font-black uppercase tracking-wider text-[#D6135F]">
              <Shield className="h-3.5 w-3.5 text-[#F0186C]" />
        ...
        {logoutMessage && (
          <div className="rounded-2xl border-2 border-emerald-200 bg-emerald-50/90 p-3.5 text-xs text-emerald-800 font-bold flex items-center gap-2.5 shadow-xs animate-fadeIn">
        ...
        {/* Authentication Card */}
        <div className="rounded-3xl border-2 border-pink-200/90 bg-white/95 backdrop-blur-md p-6 sm:p-8 shadow-xl shadow-pink-500/5 space-y-6">
          {errorMsg && (
            <div className="rounded-2xl border-2 border-rose-200 bg-rose-50 p-3.5 text-xs text-rose-900 font-bold flex items-center gap-2.5 animate-shake">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1 p-1 border border-pink-100 rounded-2xl bg-pink-50/20">
                {employees.map((emp) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2.5 p-2 rounded-xl text-left transition-all text-xs ${
                        isSelected
                          ? "bg-gradient-to-r from-pink-50 to-rose-50 border-2 border-[#F0186C] ring-2 ring-[#F0186C]/20 text-slate-900 font-bold shadow-xs"
                          : "border border-pink-100/80 bg-white hover:bg-pink-50/40 text-slate-600"
                      }`}
                    >
                      <div className="relative h-8 w-8 shrink-0 overflow-hidden rounded-lg bg-pink-100 border border-pink-200">
                        {emp.avatar ? (
            ...
            {selectedEmployee && (
              <div className="rounded-2xl border border-pink-200 bg-gradient-to-r from-pink-50/70 to-rose-50/40 p-3 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-xl border border-pink-300 bg-white shadow-xs">
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-[#D6135F] to-[#F0186C] hover:from-[#B00D4D] hover:to-[#D6135F] text-white font-black py-3 rounded-2xl shadow-lg shadow-[#F0186C]/25 text-sm gap-2 transition-all hover:scale-[1.01]"
            >
              {isLoading ? (
                <>
